...
# Model Definitions - Updated: Removed medium.en, using 'medium' for all
AVAILABLE_MODELS = {
    'tiny.en': {
        'speed': '10x', 
        'vram': '0.5GB', 
    ...
    # 'medium.en' removed as requested. 
    # Users wanting medium accuracy should select 'medium' below.
    'medium': {
        'speed': '2x', 
        'vram': '2.5GB', 
...
def find_model_directory(model_id='small.en'):
    ...
    # Primary Check: Look for specific folder
...
def resolve_channel_name(channel_name):
    """Resolve a channel name from CLI arg, with case-insensitive fallback."""
    target_dir = VIDEOS_DIR / channel_name
    if target_dir.exists():
        return channel_name, target_dir
...
"""
    ...
    channel_group.add_argument(
        '--channel', '-c', type=str, metavar='NAME',
        help='Process a specific channel by name'
    )
    channel_group.add_argument(
    ...
    parser.add_argument(
        '--model', '-m', type=str, choices=model_choices, metavar='MODEL',
        help=f'AI model to use (choices: {", ".join(model_choices)})'
    )
    parser.add_argument(
        '--task', '-t', type=str, choices=['transcribe', 'translate'],
        metavar='MODE',
        help='Processing mode: "transcribe" (keep original language) or "translate" (to English)'
    )
    parser.add_argument(
        '-y', '--yes', action='store_true',
        help='Non-interactive mode: skip all confirmation prompts and "press enter" waits. '
             'When set, --channel/--all, --model, and --task become required.'
    )
    parser.add_argument(
        '--list-channels', action='store_true',
        help='List all available channel names and exit'
    )
    parser.add_argument(
...
def list_channels_cli():
    """Print available channels and return."""
    channels = list_channels()
    if not channels:
...
def main():
    ...
    # In non-interactive mode (-y), all operational args are required
    if args.yes:
    ...
    # --- Language suffix (always auto-detect; skip the press-enter prompt in non-interactive) ---
    if args.yes:
    ...
    if choice == 'A':
        ...
        if args.channel:
            # CLI-provided channel name — resolve with case-insensitive fallback
