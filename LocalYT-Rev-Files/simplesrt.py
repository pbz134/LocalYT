...
class Subtitle:
    ...
    def _print_duration(duration) :
        ...
        # Changed comma to period for VTT format
...
class SimpleSrt:
    """
        A class to parse and manipulate Simple SubRip (SRT) and WebVTT subtitle files.

        Attributes
        ----------
        subs : List[Subtitle]
            A list of Subtitle objects representing the parsed subtitles in the input string.
        ...
        parse_timecode_string(line: str) -> Union[bool, Tuple[timedelta, timedelta]]:
            Parses a timecode string from a subtitle file and returns a tuple of start and end timedelta objects.
            If the line does not contain a valid timecode, returns False.

        parse_srt(subtitle_text: str) -> List[Subtitle]:
            Parses the input subtitle string and returns a list of Subtitle objects.
    ...
    def parse_timecode_string(self, line) -> Union[bool, Tuple[timedelta, timedelta]]:
        """
        Parses a timecode string from an SRT/VTT file and returns a tuple of start and end timedelta objects.
        If the line does not contain a valid timecode, returns False.

        :param line: string of timecode hh:mm:ss,mss --> hh:mm:ss,mss (or with periods for VTT)
        :return: tuple of timedelta objects of start and end time
        """
        # Updated regex: accepts comma or period [.,] and ignores optional VTT alignment tags .*?
        time_frame_pattern = re.compile(r"(\d+):(\d+):(\d+)[.,](\d+)\s*-->\s*(\d+):(\d+):(\d+)[.,](\d+).*?")
    ...
    def parse_srt(self, subtitle_text) :
        ...
        srtlines = [x for x in lines[start_idx:] if len(x.strip()) > 0]
        srtlines += ["", ""]
...
def dedupe_yt_srt(subs_iter):
    ...
    for subtitle in subs_iter:
        ...
        # Strip karaoke tags like <00:00:00.600><c> and closing </c> tags
        subtitle.text = re.sub(r'<[^>]+>', '', subtitle.text)
...
def process_srt(file_path, new_file_path):
    ...
    with open(new_file_path, "w", encoding="utf8") as new_file:
        # Add WEBVTT header for valid VTT file
